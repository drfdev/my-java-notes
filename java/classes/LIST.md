...
* get(int):
  **synchronized**
  Получение значение по индексу из внутреннего массива

Другое:
* equals, hashCode, toString:
  **synchronized**
  Вызывает супер-метод
...
* CopyOnWriteArrayList(Collection):
  создает копию массива из коллекции
* CopyOnWriteArrayList(E[]):
  создает копию массива из входящего массива

Добавление:
* add(E):
  Синхронизуется по lock
  делает копию массива на единицу больше и добавляет в конец элемент
* add(int, E):
  Синхронизуется по lock
  делает копию массива на единицу больше и добавляет по индексу
  Копирует массив в 2 шага: первую половину, затем вторую
* addAll(Collection):
  Синхронизуется по lock
  делает новый массив, добавляет в него предыдущие значения, а затем копирует значения из входящей коллекции
  Если коллекция пустая ничего не делает
* addAll(int, Collection):
  Синхронизуется по lock
  Делает новый массив, вставляет в серидину значения из указанной коллекции
  Занимает 2 или 3 копирования массива, в зависимости от индекса
* addAllAbsent(Collection):
  Синхронизуется по lock
  Добавляет все элементы из входящей коллекции, которые еще не содержатся в текущей коллекции
  Находит все элементы из входящей коллекции, которые еще не добавлены (по equals), затем создает копию массива и добавляет новые элементы в конец
* addIfAbsent(E):
  Если не находит по equals, то синхронизуется по lock, копирует массив с размером на 1 больше и добавляет элемент в конец
* set(int, E):
  Синхронихируется по lock
  Клонирует массив, заменяя элемент в заданной позиции
