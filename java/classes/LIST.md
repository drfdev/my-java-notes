...
* remove(int):
  ...
  Копирует массив, но на единицу меньше, вырезая элемент по индексу
  Копирует в 1 или 2 захода (если нулевой элемент удаляется, то копируется только один раз)
* remove(Object):
  Находит объект по индексу, используя equals
  Для существующего объекта синхронизуется по lock, затем делает копию массива вырезая удаленный объект
* removeAll(Collection):
  Вызывает bulkRemove
* removeIf(Predicate):
  Вызывается bulkRemove
* removeRange(int, int):
  Синхронизуется по lock
  Делает копию массива, вырезая кусок указанный в индексе
* bulkRemove(Predicate):
  private метод (есть версия package-private)
  Синхронизуется по lock
  Делает копию массива, вырезая всё что не проходит заданное условие
...
У самого интерфейса есть `static` методы:

* of()
  Возвращает пустой список (не модифицируемый, констанцу `ImmutableCollections.EMPTY_LIST`)
* of(...)
  Немодифицируемый список с переданным количество элементов (выбирается соответствующий оптимизированный класс из `ImmutableCollections`)
* copyOf(Collection)
  Копия коллекции, так же завернутая в немодифицируемый список из ImmutableCollections (используется метод `ImmutableCollections.listCopy(..)`)
